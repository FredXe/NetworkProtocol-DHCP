 ...
 * Public Methods
 *================*/
extern int arp_request(byte *dst_ip_addr);
extern int arp_reply(byte *dst_eth_addr, byte *dst_ip_addr);
extern void arp_main(netdevice_t *device, const byte *packet, u_int length);
extern int arp_send(byte *dst_ip_addr, two_bytes eth_type, byte *payload, u_int payload_len);
extern void arp_resend();
