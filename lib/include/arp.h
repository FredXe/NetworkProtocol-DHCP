...
typedef struct {
	...
	ip_addr_t src_ip_addr;			   // Source IP address
...
extern int arp_request(netdevice_t *device, const ip_addr_t dst_ip_addr);
extern int arp_reply(netdevice_t *device, byte *dst_eth_addr, const ip_addr_t dst_ip_addr);
extern void arp_main(netdevice_t *device, const byte *packet, u_int length);
extern int arp_send(netdevice_t *device, const ip_addr_t dst_ip_addr, two_bytes eth_type,
					byte *payload, u_int payload_len);
